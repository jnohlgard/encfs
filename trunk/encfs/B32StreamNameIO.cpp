...
static shared_ptr<NameIO> NewB32StreamNameIO( const Interface &iface,
	const shared_ptr<Cipher> &cipher, const CipherKey &key)
{
    return shared_ptr<NameIO>( new B32StreamNameIO( iface, cipher, key ) );
}

static bool B32StreamIO_registered = NameIO::Register("B32Stream",
	gettext_noop("Stream encoding, keeps filenames as short as possible, using Base32 encoding (for case insensitive filesystems)"),
	B32StreamNameIO::CurrentInterface(),
	NewB32StreamNameIO);
...
*/
Interface B32StreamNameIO::CurrentInterface()
{
    // implement major version 2, 1, and 0
    return Interface("nameio/b32stream", 2, 1, 2);
}

B32StreamNameIO::B32StreamNameIO( const rel::Interface &iface,
...
B32StreamNameIO::~B32StreamNameIO()
{
}

Interface B32StreamNameIO::interface() const
{
    return CurrentInterface();
}

int B32StreamNameIO::maxEncodedNameLen( int plaintextStreamLen ) const
{
    int encodedStreamLen = 2 + plaintextStreamLen;
    return B256ToB32Bytes( encodedStreamLen );
}

int B32StreamNameIO::maxDecodedNameLen( int encodedStreamLen ) const
{
    int decLen256 = B32ToB256Bytes( encodedStreamLen );
    return decLen256 - 2;
}

int B32StreamNameIO::encodeName( const char *plaintextName, int length,
	uint64_t *iv, char *encodedName ) const
{
    ...
    // convert the entire thing to base 32 ascii..
...
int B32StreamNameIO::decodeName( const char *encodedName, int length,
